...
process_inbox_boxes(Config = #{boxes := Boxes}) ->
    AllBoxes = [<<"all">>, <<"inbox">>, <<"archive">> | Boxes ],
...
build_inbox_form(HostType) ->
    ...
    AllBoxes = gen_mod:get_module_opt(HostType, ?MODULE, boxes),
    BoxOptions = [{Tag, Tag} || Tag <- AllBoxes],
    FormFields = [
                  ...
                  list_form_field(<<"order">>, <<"list-single">>, <<"desc">>, OrderOptions),
                  list_form_field(<<"box">>, <<"list-multi">>, <<"all">>, BoxOptions),
...
-spec list_form_field(Var :: binary(),
                      Type :: binary(),
                      Default :: binary(),
                      Options :: [{Label :: binary(), Value :: binary()}]) ->
    exml:element().
list_form_field(Var, Type, Default, Options) ->
...
fields_to_params(HostType, [{<<"archive">>, [Value]} | RFields], Acc) ->
    case mod_inbox_utils:binary_to_bool(Value) of
        ...
        true ->
            fields_to_params(HostType, RFields, Acc#{ box => [<<"archive">>] });
        false ->
            fields_to_params(HostType, RFields, Acc#{ box => [<<"inbox">>] })
    end;

fields_to_params(HostType, [{<<"box">>, [_|_] = Values} | RFields], Acc) ->
    case validate_boxes(HostType, Values) of
        false ->
            ?LOG_WARNING(#{what => inbox_invalid_form_field,
                           field => archive, value => Values}),
            {error, bad_request, invalid_field_value(<<"box">>, Values)};
        true ->
            fields_to_params(HostType, RFields, Acc#{ box => Values })
    end;
...
validate_boxes(HostType, Boxes) ->
    AllBoxes = gen_mod:get_module_opt(HostType, ?MODULE, boxes),
    lists:all(fun(ReqBox) -> lists:member(ReqBox, AllBoxes) end, Boxes).

invalid_field_value(Field, Values) when is_list(Values) ->
    invalid_field_value(Field, erlang:list_to_binary(Values));
invalid_field_value(Field, Value) when is_binary(Value) ->
